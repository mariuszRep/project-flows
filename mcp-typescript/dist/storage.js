...
export function createTask(title, createdBy = "system") {
    const now = new Date();
    const task = {
        id: taskIdCounter++,
        title,
...
export function addBlock(taskId, blockType, content) {
...
export function updateBlock(taskId, blockType, content) {
    const taskBlocks = blockStorage.get(taskId);
    if (!taskBlocks)
        return null;
...
export function getBlocks(taskId) {
    return blockStorage.get(taskId) || [];
}
export function getBlockByType(taskId, blockType) {
    const blocks = getBlocks(taskId);
    return blocks.find(b => b.block_type === blockType) || null;
}
// Unified output aggregator function
export function aggregateFinalOutput(taskOutput, blocksOutput, metadataOutput) {
