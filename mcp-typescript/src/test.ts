...
async function runTests() {
  ...
  console.log("🔗 Dependencies:", Object.entries(dynamicProperties).map(([name, config]) => 
    `${name}: [${config.dependencies?.join(', ') || 'none'}]`).join(', '));
  console.log("📊 Execution order:", Object.entries(dynamicProperties).map(([name, config]) => 
    `${name}:${config.execution_order}`).join(', '));
  console.log();
