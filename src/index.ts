...
// Type definitions for the markdown script structure
interface MarkdownBlock {
  [key: string]: string;
}

interface MarkdownScript {
  title: string;
  stage: string;
...
class MarkdownScriptGenerator {
      ...
      try {
        switch (name) {
          case 'generate_markdown_script':
            return await this.generateMarkdownScript(args as any);
          case 'create_default_script':
            return await this.createDefaultScript(args as { title?: string; includeOptionalSections?: boolean });
          case 'generate_plan_from_prompt':
            return await this.generatePlanFromPrompt(args as any);
          default:
  ...
  private async generateMarkdownScript(args: GenerateMarkdownArgs) {
  ...
  private async generatePlanFromPrompt(args: GenerateFromPromptArgs) {
  ...
  private generateContentFromInstruction(sectionName: string, instruction: string, appTitle: string): string {
  ...
  private createTemplateFromPrompt(prompt: string): MarkdownScript {
  ...
  private async createDefaultScript(args: { title?: string; includeOptionalSections?: boolean }) {
    const { title = 'Generated Script', includeOptionalSections = false } = args;

    const defaultScript: MarkdownScript = {
      title,
      stage: 'Draft',
      blocks: [
        {
...
export class MarkdownScriptTester {
  static async testGeneration() {
    ...
    const customScript: MarkdownScript = {
